...
public class Golem2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(Knight.vidas<=0){
            NoPerseguir();
            anim.SetBool("Golem2Ataca", false);
        } 

        if(vidasGolemG<=0){
            isLive=false;
            rb2d.velocity = new Vector2(0,2); 
            anim.SetBool("Golem2Moviendose", false); 
            NoPerseguir();  
            anim.SetBool("Golem2Ataca", false);
            anim.SetBool("Golem2Muere", true); 
    ...
    {
        anim.SetBool("Golem2Moviendose", true); 
